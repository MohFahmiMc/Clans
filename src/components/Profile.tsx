...
export default function Profile({ member, onClose, getRoleColor, getSpecialIcon }: ProfileProps) {
  ...
  const getCustomBackgroundStyle = () => {
    if (!userTheme || userTheme.trim() === '') {
      return { background: '#0d0d0e' };
    }
    
    const theme = userTheme.trim();
    // Jika input mengandung kata 'gradient', gunakan langsung sebagai CSS background
    if (theme.includes('gradient')) {
      return { background: theme };
    }
    
    // Jika berupa warna hex/rgb, buatkan efek degradasi gradient gelap ala Discord Nitro
    return {
      background: `linear-gradient(180deg, ${theme} 0%, #0a0a0c 85%)`
    };
  };
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-3 sm:p-4 md:p-6 bg-black/80 backdrop-blur-md transition-all duration-300">
      ...
      <div 
        className="relative w-full max-w-sm sm:max-w-xl md:max-w-2xl lg:max-w-3xl rounded-2xl border border-white/15 overflow-hidden shadow-[0_0_70px_rgba(0,0,0,0.95)] my-auto animate-in fade-in zoom-in-95 duration-200 flex flex-col max-h-[90vh] sm:max-h-[85vh] z-10 transition-all"
      ...
      >
        ...
        <div className="overflow-y-auto scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
          ...
          >
            {/* Gradient Mask Antara Banner & Body Profil */}
          ...
          <div className="px-4 sm:px-6 md:px-8 lg:px-10 pb-6 sm:pb-8 md:pb-10 relative">
            ...
            <div className="flex items-end justify-between -mt-12 sm:-mt-16 md:-mt-20 relative z-10 mb-4 sm:mb-6">
              ...
              <div className="group relative">
                <div className="w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 lg:w-28 lg:h-28 rounded-2xl bg-black/80 border-2 sm:border-4 border-white/10 overflow-hidden shadow-2xl group-hover:border-amber-500 transition-all duration-300 flex-shrink-0 relative">
                  <div 
            ...
            <div className="flex flex-col md:flex-row md:justify-between md:items-end gap-4 mt-1 sm:mt-2">
              <div className="space-y-2">
                ...
                <span className={`text-[10px] sm:text-xs font-black uppercase tracking-widest inline-block px-3 py-1 rounded-md border shadow-sm backdrop-blur-md ${roleStyle}`}>
                  Clan {member.role}
                </span>
              </div>
              ...
              {member.specialRoles && member.specialRoles.length > 0 && (
                <div className="flex flex-wrap gap-2 p-2.5 sm:p-3 bg-black/50 rounded-xl border border-white/10 backdrop-blur-md shadow-lg">
                  {member.specialRoles.map((role, i) => {
                    ...
                    return (
                      <div 
                        key={i} 
                        className="flex items-center gap-2 bg-black/60 hover:bg-black/90 px-2.5 py-1.5 rounded-lg border border-white/10 hover:border-white/20 transition-all duration-200"
                      >
                        <img 
                          src={iconSrc} 
                          alt={role} 
            ...
            <div className="mt-6 sm:mt-8 pt-4 sm:pt-6 border-t border-white/10">
              <h3 className="text-[11px] sm:text-xs font-bold uppercase tracking-widest text-slate-300 mb-2">
                Tentang Player
              </h3>
              <div className="bg-black/40 backdrop-blur-md p-4 sm:p-5 rounded-xl border border-white/10 shadow-inner">
